...
export const OPCO_FORMULAS: Record<string, TooltipInfo> = {
  ...
  ipRev: {
    ...
    formula:
      "Inpatient Revenue = (Inpatient Cases * (ipRevenue Assumption * priceMultiplier)) / 1,000 [in Billion IDR]\n(Note: PriceMultiplier escalates at priceIncYears1_6 then priceIncYears7_plus)",
  },
  opRev: {
    desc: "Estimated gross revenue generated from outpatient visits, outpatient diagnostics, and day-care clinics.",
    formula:
      "Outpatient Revenue = (Outpatient Visits * (opRevenue Assumption * priceMultiplier)) / 1,000 [in Billion IDR]",
  },
  totalRev: {
  ...
  totalMedSupp: {
    ...
    formula:
      "Medical Supplies = ((Inpatient Cases * ipMedSupply + Outpatient Visits * opMedSupply) * (1 + medSupplyInf%)^(Year - 1)) / 1,000 [in Billion IDR]",
  },
  totalDocFee: {
    desc: "Proportion of inpatient and outpatient gross clinical revenues paid as fees/commissions to physicians.",
    formula:
      "Doctor Fees = (docFeeIp% * Inpatient Revenue) + (docFeeOp% * Outpatient Revenue)",
  },
  grossProfit: {
  ...
  staffCost: {
    ...
    formula:
      "Staff Cost = (monthlyStaffCost * 12) * (1 + staffInf%)^(Year - 1)",
  },
  recurringOpex: {
    desc: "Other Clinical General & Admin support OPEX, calculated as percentage allocations of total Net Revenue.",
    formula:
      "Other OpEx = (adminExpRate + utilExpRate + marketingExpRate + maintExpRate)% * Net Revenue",
  },
  ebitdar: {
  ...
  rent: {
    ...
    formula:
      "Calculated via lease structures: Fixed Rent escalation % or EBITDA-linked rent formulas.",
  },
  ebitda: {
  ...
  cumNI: {
    ...
    formula:
      "Cumulative Net Income (Year t) = Cumulative Net Income (Year t-1) + Net Income (Year t)",
  },
  distributableProfit: {
  ...
  cumulativeRetainedEarnings: {
    ...
    formula:
      "Cumulative Retained Cash (Year t) = Cumulative Cash (Year t-1) + Retained Profit (Year t)",
  },
  ev: {
    desc: "Operating enterprise appraisal valuation at designated exit year.",
    formula:
      "Enterprise Value = Stabilized Operational EBITDA * exitMultiple (or Cap Rate equation if designated)",
  },
  opCoExit: {
  ...
  pA_Outlay: {
    ...
    formula:
      "Strategic Outlay = OpCo Total Investment * 51% (Negative during setup years)",
  },
  pB_Outlay: {
    desc: "Capital contribution (outlay) apportioned to Vasanta (49%) during setup/pre-op years.",
    formula:
      "Vasanta Outlay = OpCo Total Investment * 49% (Negative during setup years)",
  },
  shareA: {
  ...
  pA_Net: {
    ...
    formula:
      "Strategic Net Flow = Strategic Outlay + Strategic Dividend + Strategic Exit",
  },
  pB_Net: {
    desc: "Vasanta's net look-through cash yield including outlays, dividends, and exit proceeds.",
    formula:
      "Vasanta Net Flow = Vasanta Outlay + Vasanta Dividend + Vasanta Exit",
  },
};
...
export const PROPCO_FORMULAS: Record<string, TooltipInfo> = {
  ...
  overheadOpex: {
    ...
    formula:
      "Overhead Expense = Flat monthly charge or percentage scaled rate based on property assets",
  },
  preOpeningDev: {
  ...
  gop: {
    ...
    formula:
      "GOP = Revenue - Operating Expenses (Maintenance, Property Taxes, Overhead, MedEq Leases)",
  },
  ebitda: {
  ...
  dep: {
    ...
    formula:
      "Under Indonesian PSAK 16 & PSAK 19 straight-line depreciation conventions representing asset decay.",
  },
  ebit: {
  ...
  principal: {
    ...
    formula:
      "Principal Repaid = Monthly mortgage amortization PMT - Interest portion",
  },
  dscr: {
  ...
  deferredCapex: {
    ...
    formula:
      "Acquisition Buyout = Leased Med. Equipment Value (occurring at buyout year)",
  },
  grossExitValue: {
  ...
  netExitProceeds: {
    ...
    formula:
      "Property Exit Yield = Facility Valuation - Remaining Debt - Land Cost (if separate) - Selling Costs",
  },
  opFcfe: {
    ...
    formula:
      "FCFE (Op) = Net Income + Depreciation - Principal - Deferred Capex",
  },
  fcfe: {
  ...
  cumFcfe: {
    ...
    formula:
      "Cumulative FCFE (Year t) = Cumulative FCFE (Year t-1) + FCFE (Year t)",
  },
  interestExLand: {
  ...
  netExitProceedsExLand: {
    ...
    formula:
      "Ex-land Exit Return = Valuation - Outstanding Debt Ex-Land - Land value deduction - transactional cuts",
  },
  opFcfeExLand: {
    desc: "Operational free cash flows (excluding land) before exit proceeds.",
    formula:
      "FCFE Op (Ex-Land) = Net Income Ex-Land + Dep - Principal Ex-Land - Deferred Purchases",
  },
  fcfeExLand: {
  ...
  cumFcfeExLand: {
    ...
    formula:
      "Cumulative FCFE Ex-Land (Year t) = Cumulative Year (t-1) + FCFE Ex-Land (Year t)",
  },
};
...
export const CONSOLIDATED_FORMULAS: Record<string, TooltipInfo> = {
  ...
  opCoInvestmentFlow: {
    ...
    formula:
      "Matches the 'Outlay' of Partner B in Year 1 and Year 2 in OpCo Cascade.",
  },
  opCoOperatingDividendFlow: {
    desc: "Partner B's share (49%) of OpCo distributed clinical dividends from profitable operations.",
    formula:
      "Matches 'Dividend' of Partner B swept to look-through combined sponsors.",
  },
  opCoExitFlow: {
  ...
  netFlow: {
    ...
    formula:
      "Combined Cash Flow = PropCo FCFE (100%) + OpCo Dividends (49%) + OpCo Exit Proceeds (49%)",
  },
  cumCf: {
    desc: "Consolidated Cumulative net return position achieved by lookup sponsors across the model duration.",
    formula:
      "Cumulative Combined cash (Year t) = Cumulative (Year t-1) + Combined Cash Flow (Year t)",
  },
  lookThroughRevenue: {
    desc: "GAAP consolidated revenue representing physical rental income and strategic share of clinical operations.",
    formula:
      "Look-Through Revenue = OpCo Net Revenue * 49% + PropCo Rental Revenue * 100%",
  },
  lookThroughEbitda: {
